...
class VespaInlineSettings:
    ...
    def __init__(self):
        ...
        # err - text out of filenames and traceback
        # xml = full fit provenance
        # pdf - hires image of plot/table fit results
        # png - fit result image of plots/table, same as DICOM output
        # dcm - 'screenshot' fit result image of plots/table
        ...
        self.dcm_pdf_fname = os.path.join(self.output_dir,'default_output_dicom_pdf.dcm')
...
def analysis_kernel(params, verbose=False, process_id='single', exception_as_list=False):
    ...
    try:
        ...
        if verbose:
            if isinstance(fdatasets['metab'], basestring):
                print(process_id + ' - Begin - metab filename = ' + fdatasets['metab'])
            else:
                print(process_id + ' - Begin - metab filename = inline')
        msg = 'begin loading presets'
        presets = []
        for key in ['metab','water','coil','ecc']:
            fname = fpresets[key]
        ...
        if settings.dataformat in ['philips_press28_dicom', 'philips_slaser30_cmrr_spar', 'vasf']:
            ...
            if fname is not None:
                dataset = util_file_import.get_datasets_cli(fname, settings.import_class, None)
                datasets.append(dataset[3], dataset[2], dataset[0],dataset[1],)            # all files in one

        elif settings.dataformat in ['inline_siemens',]:
            datasets = [fdatasets['metab'], fdatasets['water'], fdatasets['coil'], fdatasets['ecc']]
        ...
        if verbose:
            if isinstance(fdatasets['metab'], basestring):
                ...
                print(process_id + ' - Finish - metab filename = inline')
