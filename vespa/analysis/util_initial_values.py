...
import vespa.analysis.lowess as lowess
...
# import vespa.analysis.util_file as util_file as ufile       # used for debug

from vespa.analysis.xy_b0_correction  import b0_correction
from vespa.analysis.xy_auto_correlate import auto_correlate
from vespa.analysis.xy_savitzky_golay import savitzky_golay
from vespa.analysis.xy_optimize_phase import optimize_phase0_correlation
from vespa.analysis.xy_optimize_phase import optimize_phase1_correlation
from vespa.analysis.xy_optimize_phase import optimize_phase0_integration
from vespa.analysis.xy_optimize_phase import optimize_phase1_integration
...
#                print ' *ko peak_3ppm_amp initval area = '+str(peak_3ppm_amp)
    ...
    if set.initial_small_peak_areas == 'peak_2ppm' :
        ...
        ctr = util_ppm.ppm2pts(2.01, dataset)     # in pts here
...
#        print ' *** peak_2ppm_amp initval area = '+str(peak_2ppm_amp)
    ...
    if peak_3ppm_amp is None and set.initial_small_peak_areas == 'peak_3ppm' : 
        ...
        ctr = util_ppm.ppm2pts(3.02, dataset)     # in pts here
    ...
    if set.macromol_model == FitMacromoleculeMethod.SINGLE_BASIS_DATASET:
        ...
        if set.macromol_single_basis_dataset_initval == FitMacromoleculeMethodInitVal.MANUAL:
            ...
            if chain.basis_mmol is not None:
                ...
                ppm_start = int(util_ppm.ppm2pts(ppm_start, dataset, acq=False))
                ppm_end   = int(util_ppm.ppm2pts(ppm_end  , dataset, acq=False))
