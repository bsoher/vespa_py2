...
def _pretty_space_text(left='', middle='', right='', total_width=77):
    ...
    """
    pad1 = u" "*((total_width - len(middle))//2)        # 145 spaces found empirically
    pad2 = u" "*((total_width - len(middle))%2)         # deal with odd number of pad1 spaces
...
def png_error(errmsg,
    ...
    msg1 = _pretty_space_text(u"Vespa-Analysis Version: %s" % (vespa_version,), middle,
                              u"Processing Timestamp: %s" % (timestamp,))

    plt.figtext(0.042, 0.94, msg1,
                wrap=True,
                horizontalalignment='left',
                fontsize=8,
                fontname=fontname)

    msg = u"Error Message : %s " % (errmsg, )
    plt.figtext(0.042, 0.85, msg,
                wrap=True,
                horizontalalignment='left',
                fontsize=9,
                fontname=fontname)
...
def lcm_like(   dataset,
    ...
    if quantvals:
        if 'quant' in dataset.blocks.keys():
    ...
    middle = u"Fitted - Full Model (Phase0/1 Corrected)" if fixphase else u"Fitted - Full Model"
    msg1 = _pretty_space_text(u"Vespa-Analysis Version: %s" % (vespa_version, ), middle, u"Processing Timestamp: %s"   % (timestamp, ))
...
def lcm_multipage_pdf( dataset, 
    ...
    for i in range(yfind.shape[0] + 1):
        ...
        if i == 0:          # general plot
            ...
            if fixphase:
                middle = u"Fitted - %s (Phase0/1 Corrected)" % (prior_list[i-1], )
            else:
                middle = u"Fitted - %s" % (prior_list[i-1],)
        ...
        plt.figtext(0.042, 0.94, msg1, wrap=True, horizontalalignment='left', fontsize=8, fontname=fontname)
        msg2 = u"Data Source : %s \nVIFF File   : %s" % (data_source, viffname)
        plt.figtext(0.042, 0.89, msg2, wrap=True, horizontalalignment='left', color=local_grey, fontsize=8, fontname=fontname)
#         msg2 = u"VIFF File   : %s \nData Source : %s" % (viffname, data_source)
#         plt.figtext(0.042, 0.89, msg2, wrap=True, horizontalalignment='left', color=local_grey, fontsize=10, fontname=fontname)
...
def analysis_plot2(dataset, viffpath='', 
    ...
    middle = u"'Fit' Tab Results  (Phase0/1 Corrected)" if fixphase else u"'Fit' Tab Results"
    msg1 = _pretty_space_text(u"Vespa-Analysis Version: %s" % (vespa_version, ), middle, u"Processing Timestamp: %s"   % (timestamp, ))
...
#        fmajor = np.round(fmajor,0)
    ...
    middle = u"Inline Processing (Ph0/1 Corr)" if fixphase else u"Inline Processing"
    msg1 = _pretty_space_text(u"Vespa-Analysis v%s" % (vespa_version, ), middle, u"Timestamp: %s"   % (timestamp, ), total_width=48)
...
def debug_multipage_pdf( dataset, 
    ...
    for i in range(yfind.shape[0] + 1):
        ...
        if i == 0:          # general plot
            ...
            plt3 = rsd
            lbl  = 'Data, Fit+Base, Resid'
            middle = u"Fitted - Full Model"  if fixphase else u"Fitted - Full Model (Phase0/1 Corrected)"
        else:
            plt1 = fre
            plt2 = yfi[i-1,:]
            plt3 = None
